	...
	session_start();
		...
		class dbFunction extends dbConnect{
			...
			public function Login($username,$password){
				$dbConn = $this->conn();
				$query = mysqli_query($dbConn,"SELECT * FROM user where username = '".$username."' AND password = '".md5($password)."' AND status='ACTIVE'");
			...
			public function updatemyaccount($id,$email,$pass,$fname,$lname,$mname,$address,$Company,$Contactno,$position,$status){
				$dbConn = $this->conn();
				if($pass == ""){
					$Update = mysqli_query($dbconn,"Update user SET 
						email = '$email',
						fname = '$fname',
						lname = '$lname',
						mname = '$mname',
			...
			public function checkemail($email,$email2){
				...
				{
					$check = mysqli_query($dbConn,"SELECT * FROM user WHERE email = '".$email."'");  
			...
			public function adduser($username,$email,$pass,$fname,$lname,$mname,$address,$Company,$Contactno,$position,$Status){
				$dbConn = $this->conn();
				$md5pass = md5($pass);
				mysqli_query($dbConn,"Insert into user(
					username,
					email,
					password,
					fname,
			...
			public function user($username){
				$dbConn = $this->conn();
				$check = mysqli_query($dbConn,"SELECT * FROM user where username = '".$username."'");
				$row = mysqli_num_rows($check);
				if($row > 0){  
		                return true;  
		            } 
			...
			public function delete($id){
				$dbConn = $this->conn();
				mysqli_query($dbConn,"DELETE from user where id=$id");
				return TRUE;
			}
